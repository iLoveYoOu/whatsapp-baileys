...
const { PixRouteRegistry } = require('./src/pix-route-registry');
const { SupabasePixRouteRegistry } = require('./src/supabase-pix-route-registry');
const { createPixRouteRouter } = require('./src/pix-route-api');
...
const PIX_ROUTES_V1_ENABLED = process.env.ARTAUTO_PIX_ROUTES_V1_ENABLED === 'true';
const PIX_ROUTES_ADMIN_TOKEN = process.env.ARTAUTO_PIX_ROUTES_ADMIN_TOKEN || '';
const PIX_ROUTES_PEPPER = process.env.ARTAUTO_PIX_ROUTES_PEPPER || '';
const PIX_ROUTES_STORAGE = process.env.ARTAUTO_PIX_ROUTES_STORAGE || 'memory';

if (PIX_ROUTES_V1_ENABLED) {
  try {
    const pixRouteRegistry = PIX_ROUTES_STORAGE === 'supabase'
      ? new SupabasePixRouteRegistry({
          url: process.env.ARTAUTO_PIX_SUPABASE_URL || process.env.SUPABASE_URL,
          serviceRoleKey: process.env.ARTAUTO_PIX_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY,
          pepper: PIX_ROUTES_PEPPER,
          destinationKey: process.env.ARTAUTO_PIX_DESTINATION_KEY
        })
      : new PixRouteRegistry({ pepper: PIX_ROUTES_PEPPER });
    if (PIX_ROUTES_STORAGE === 'memory') {
      for (const [slug, destinationRef] of Object.entries(DESTINOS_PIX)) {
        pixRouteRegistry.createRoute({
          slug, publicName: slug.charAt(0).toUpperCase() + slug.slice(1),
          destinationRef, legacyAlias: slug
        });
      }
    }
    app.use('/api/v1', createPixRouteRouter({
      registry: pixRouteRegistry,
      adminToken: PIX_ROUTES_ADMIN_TOKEN,
      adminUsername: process.env.ARTAUTO_PIX_ADMIN_USERNAME,
      adminPasswordHash: process.env.ARTAUTO_PIX_ADMIN_PASSWORD_HASH,
      sessionSecret: process.env.ARTAUTO_PIX_ADMIN_SESSION_SECRET,
      deliver: async event => {
        if (!sock) throw new Error('DELIVERY_UNAVAILABLE');
        const mensagem = String(event.payload?.texto || '');
        const nome = mensagem.match(/^(.*?) te enviou um Pix/i)?.[1]?.trim() || 'Desconhecido';
        const valor = mensagem.match(/R\$\s*([\d.,]+)/i)?.[1]?.trim() || '0,00';
        const registroFraude = buscarNaBlacklist(nome);
        await sock.sendMessage(event.destinationRef, {
          text: msgPixRecebido(nome, valor, Boolean(registroFraude))
        });
      }
    }));
    console.log(`[PixRoutesV1] API habilitada em /api/v1 (${PIX_ROUTES_STORAGE}).`);
  } catch (error) {
    console.error(`[PixRoutesV1] Inicialização recusada: ${error.message}`);
  }
}
...
const lucroTabela = {
  ...
  1450: 165, 1500: 180, 1600: 190, 1650: 195,
